...
const seed = async () => {
  try {
    ...
    const categoryMap = new Map(
      categoryDocs.map((category) => [category.name, category]),
    );
    ...
    const restaurantDocs = await Restaurant.insertMany([
      {
        ...
        menuItemIds: [] as mongoose.Types.ObjectId[],
      },
      {
    ...
    const restaurantMap = new Map(
      restaurantDocs.map((restaurant) => [restaurant.name, restaurant]),
    );
    ...
    const menuItemsByRestaurant = new Map<string, mongoose.Types.ObjectId[]>();
    ...
    const order3 = await Order.create({
      ...
      orderItems: [
        {
          quantity: 1,
          menuItemId: menuItems.find(
            (m) => m.name === "Chicken Teriyaki Bento",
          )!._id,
        },
        {
          quantity: 1,
